...
def harmonik_analiz(df):
    ...
    l2 = df[["ssh"]]  # en sonda kullanmak için aldım
    ...
    df3 = df2.copy()
    df3.drop(columns=["ssh"], inplace=True)  # l matrisini dropluyoruz buradan
    ...
    df_merged = fxt.dates_interpolation(df_merged)

    # Ana ölçülerile birleştirilme
    df_merged = df_merged.join(l2)
    df_merged.rename(columns={"SSH": "SSH_model", "ssh": "SSH_ilk"}, inplace=True)
    ...
    A1 = abs(round(A1, 1))  # Yuvarlama işlemi, sanırım abs değeri alınmalı !!!!!!!
    ...
    B1 = abs(round(B1, 1))  # Yuvarlama işlemi, sanırım abs değeri alınmalı !!!!!!!
    ...
    A2 = abs(round(A2, 1))  # Yuvarlama işlemi, sanırım abs değeri alınmalı !!!!!!!
    ...
    B2 = abs(round(B2, 1))  # Yuvarlama işlemi, sanırım abs değeri alınmalı !!!!!!!
    ...
    annual_genlik = round(m.sqrt(A2 ** 2 + B2 ** 2), 1)  # cm biriminde
    annual_faz = round(m.atan(B2 / A2), 1)
    ...
    """
    info = f"""----------------------------------------------------------------------------------------
            MSS ve hatası: {mss} m ± {mss_hata} cm
            Trend ve hatası: {trend} ± {trend_hata} mm/yıl
            Ak1 ve hatası: {A1} ± {A1_hata} cm
            Bk1 ve hatası: {B1} ± {B1_hata} cm
            Ak2 ve hatası: {A2} ± {A2_hata} cm
            Bk2 ve hatası: {B2} ± {B2_hata} cm
            Semi-annual Genlik ve Faz: {semi_annual_genlik} cm - {semi_annual_faz}
            Annual Genlik ve Faz: {annual_genlik} cm - {annual_faz}
----------------------------------------------------------------------------------------"""

    return info, df_merged, mss, plota_trend
