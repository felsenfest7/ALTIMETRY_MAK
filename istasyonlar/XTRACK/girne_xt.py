...
dataset = fxt.read_xt(data, 35.4937203, 32.95539255, 0.40)

#Ardından kıyıya uzak ve yakın iki XTRACK altimetri ölçme noktasının belirlenmesi
dataset1 = dataset[dataset["points_numbers"] == 69]     #kıyıya uzak olan
dataset2 = dataset[dataset["points_numbers"] == 68]     #kıyıya yakın olan
...
dataset1_pre_ekksa = fxt.pre_ekksa(dataset1_aylık[1], "2008-10-01", "2023-12-01")
...
dataset1_grafikler = pf.plot_digerleri_xt(df1, df2, df3, df4, "Girne (Uzak)")
dataset1_ekksa_grafik = pf.plot_ekksa_xt(df5, "Girne (Uzak)", dataset1_ekksa[2], dataset1_ekksa[3], 2020, 12, 1, 26.453)
...
dataset2_pre_ekksa = fxt.pre_ekksa(dataset2_aylık[1], "2008-10-01", "2023-12-01")
...
dataset2_grafikler = pf.plot_digerleri_xt(df6, df7, df8, df9, "Girne (Yakın)")
dataset2_ekksa_grafik = pf.plot_ekksa_xt(df10, "Girne (Yakın)", dataset2_ekksa[2], dataset2_ekksa[3], 2020, 12, 1, 26.735)

#Mesafeler ve koordinatlar
dataset1_distance = fxt.mesafe_hesapla(35.34082452, 33.33406448, dataset1.lat.mean(), dataset1.lon.mean())
dataset2_distance = fxt.mesafe_hesapla(35.34082452, 33.33406448, dataset2.lat.mean(), dataset2.lon.mean())
